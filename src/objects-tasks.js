...
function mergeObjects(objects) {
  ...
  objects.forEach((object) => {
    keys.push(Object.entries(object));
  });
  const flatKey = keys.flat();
  flatKey.forEach((pair) => {
...
function sellTickets(queue) {
  class Seller {
    constructor(initialSellerMoney, ticketCost) {
      this.money = initialSellerMoney;
      this.canWork = true;
      this.cost = ticketCost;
    }

    operate(bill) {
      if (bill - this.cost > this.money && this.canWork) {
        this.canWork = false;
        return false;
      }
  ...
  const seller = new Seller(0, 25);
  return queue.every((bill) => seller.operate(bill));
}
...
function group(array, keySelector, valueSelector) {
  ...
  array.forEach((item) => {
    if (citiesMap.has(keySelector.call(item, item))) {
      citiesMap.set(keySelector.call(item, item), [
        ...citiesMap.get(keySelector.call(item, item)),
        valueSelector.call(item, item),
      ]);
    } else {
      citiesMap.set(keySelector.call(item, item), [
        valueSelector.call(item, item),
